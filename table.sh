...
LC_CHANGECOUNT=""
LC_CHANGECOUNT_STR=""
grep GLIBC_VERS $PREFIX/*/run.out|sed 's.[/=]. .g'|sort -k5 -V|awk '{print$2}' | while read AMI
do
  ...
  if [ -n "$PREV_AMI" ]; then
    ...
    diff $PREFIX/$PREV_AMI/unicode-${UNICODE_VERS}-chars-sorted-glibc-${PREV_GLIBC_VERS}.txt $PREFIX/$AMI/unicode-${UNICODE_VERS}-chars-sorted-glibc-${GLIBC_VERS}.txt |awk '/^[<>]/{print$3" "$2" "$1 pos;next}!/^---/{pos=$1}'|sort >>"${CHANGELIST}"
    ...
    diff -yt --suppress-common-lines $PREFIX/$PREV_AMI/LC_COLLATE $PREFIX/$AMI/LC_COLLATE >"${LC_CHANGELIST}"
    ...
    LC_CHANGECOUNT_STR=""; (( LC_CHANGECOUNT > 0 )) && LC_CHANGECOUNT_STR="[$LC_CHANGECOUNT]($LC_CHANGELIST)"
