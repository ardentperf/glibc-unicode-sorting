...
do
  ...
  if [ -n "$PREV_AMI" ]; then
    ...
    echo "" >>$SUMMARY
    awk '{print$1" "$2" "$3}' $BASEDIR/changelist1.tmp|sort -u|perl -CSDA -e'
      use strict;
      use warnings;
      use utf8;
    ...
    sort -u $BASEDIR/changelist1.tmp|perl -CSDA -e'
      use strict;
      use warnings;
      use utf8;
      my %filter;
    ...
    CHANGECOUNT=$(awk '{print$1}' $BASEDIR/changelist1.tmp|sort -u|wc -l)
    ...
    grep -E "^Only in $PREFIX/$AMI" $LC_CHANGELIST|awk '{print" "$4" (removed)"}' >>langs.tmp
    ...
    CHANGELIST_SIZE=$(stat $STATARG $CHANGELIST)
    ...
    if (( SORTDIFF_SIZE > 40000000 )); then
      ...
      echo "" >>$SORTDIFF
      head -c40000000 $BASEDIR/diff.tmp >>$SORTDIFF
    fi
  ...
  case $PREFIX in
    _ubuntu|_rhel)
      ...
      ;;
    _ubuntu-icu)
      echo "| $ICU_VERS | $OS_VERS | $SUMMARY_STR_en | $CHANGECOUNT_STR_en | $SUMMARY_STR_ja | $CHANGECOUNT_STR_ja | $SUMMARY_STR_zh | $CHANGECOUNT_STR_zh | $SUMMARY_STR_ru | $CHANGECOUNT_STR_ru | $SUMMARY_STR_fr | $CHANGECOUNT_STR_fr | $SUMMARY_STR_de | $CHANGECOUNT_STR_de | $SUMMARY_STR_es | $CHANGECOUNT_STR_es | [$AMI]($PREFIX/$AMI) |"
      ;;
  esac
  PREV_AMI="$AMI"
